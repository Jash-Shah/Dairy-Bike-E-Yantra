...
function [t,y] = sim_RW_pendulum(m1, m2, l1, l2, wr, g, y0)
  tspan = 0:0.1:10;                  ## Initialise time step           
  u = 0;                             ## No Input
  [t,y] = ode45(@(t,y)RW_pendulum_dynamics(y, m1, m2, l1, l2, wr, g, u),tspan,y0);
...
function [A, B] = RW_pendulum_AB_matrix(m1 , m2, l1,l2, wr, g)
  I1= 0.8045; # Moment of Inertia of pendulum bar
  I2= 0.2785; # Moment of Inertia of reaction wheel
  a = m1*(l1^2) + m2*(l2^2) + I1;
  b = (m1*l1 + m2*l2)*g;
...
function [t,y] = pole_place_RW_pendulum(m1, m2, l1, l2, wr, g, y_setpoint, y0)
  [A,B] = RW_pendulum_AB_matrix(m1 , m2, l1,l2, wr, g);
  #disp((ctrb(A,B))); 
  eigs = [-2;-2;-5;-5];
  K = place(A,B,eigs);
  tspan = 0:0.1:10;
  [t,y] = ode45(@(t,y)RW_pendulum_dynamics(y, m1, m2, l1,l2, wr, g, -K*(y-y_setpoint)),tspan,y0);
#  disp(y);
endfunction
...
function [t,y] = lqr_RW_pendulum(m1, m2, l1,l2, wr, g, y_setpoint, y0)
  ...
  R = 16.6*[1];
  sys = c2d(ss(A,B),0.1);
  disp(isstabilizable(sys));
  K = lqr(A,B,Q,R);
  tspan = 0:0.1:20; # Time Array 
  [t,y] = ode45(@(t,y)RW_pendulum_dynamics(y, m1, m2, l1,l2, wr, g, -K*(y - y_setpoint)),tspan,y0);  # ODE solver to solve differential equations
  #disp(y);
  endfunction
...
function RW_pendulum_main()
  m1 = 15.048;     # Mass of Pendulum Bar
  m2= 4.952;   # Mass of Reaction Wheel 
  l1= 0.231;      # Dist of COM of Bike from Origin
  l2 = 0.317; #Dist of COM of RW from Origin
  g = 9.8;    # Centre of Gravity
  y0 = [0.9*pi; 0; 0; 0]; # Initial Conditions
  y_setpoint = [pi; 0;pi; 0];  # Reference Point
  wr=0.3; #wheel radius

## Function Calls for different control techniques for stabilizing RW Pendulum
  
## [t,y] = sim_RW_pendulum(m1, m2, l1,l2, wr, g, y0);
##    [t,y] = pole_place_RW_pendulum(m1, m2, l1,l2, wr, g, y_setpoint, y0);
[t,y] = lqr_RW_pendulum(m1, m2, l1,l2, wr, g, y_setpoint, y0);
  
  for k = 1:length(t)
    draw_RW_pendulum(y(k, :), m1, m2, l2, wr);  # Function to draw current state of RW Pendulum
  endfor
endfunction
